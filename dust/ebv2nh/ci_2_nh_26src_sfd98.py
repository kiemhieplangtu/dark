...
def cal_nh_from_dust(map_file, info, lownhi):
	...
	nh, nher   = nh_from_ebv(ci_map, info)
	xnh, lnher = nh_from_ebv(ci_map, lowhi)	
	...
	plt.plot([0,40],[0,40], 'k--', label=r'$N_{H} = N_{HI}, N_{H} = 5.8\cdot10^{21}[cm^{-2}mag^{-1}]\cdot E(B-V), SFD 1998$')
	...
	plt.text(15., 4., r'E(B-V) from Schlegel, Finkbeiner & Davis (SFD), 1998', color='k', fontsize=17)
