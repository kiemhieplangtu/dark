...
def cal_nh_from_dust(map_file, info, lowhi):
	...
	# TTYPE2  = 'ERR_TAU '           / Error on optical depth                         
	# TTYPE3  = 'EBV     '           / E(B-V) color excess                            
	# TTYPE4  = 'EBV_ERR'            / Integrated emission                            
	# TTYPE5  = 'TEMP    '           / Dust equilibrium temperature                   
	# TTYPE6  = 'ERR_TEMP'           / Error on T                                     
	# TTYPE7  = 'BETA    '           / Dust emission spectral index                   
	# TTYPE8  = 'ERR_BETA'           / error on Beta  
	...
	# plt.plot(nhi,nh, 'rd', label='data of 94 LOS', ms=10)
	plt.errorbar(nhi,nh,xerr=nhier, yerr=nher, color='r', marker='o', ls='None', markersize=8, markeredgecolor='b', markeredgewidth=1, label='data')
	plt.plot([0,200],[0,200], 'k--', label='$N_{H} = N_{HI}$')
	plt.title('$N_{H}$ and $N_{HI}$ along 94 lines-of-sight', fontsize=30)
	...
	plt.text(100., 40., r'E(B-V) from Planck data R1.1', color='k', fontsize=17)
