...
## Read nh (derived from CO), nhi and errors #
 ...
 # version 9/2016
 # Author Van Hiep ##
def read_dust_nh_nhi(fname = 'nh_16src_from_dust.txt'):
	cols = ['indx', 'src', 'l', 'b', 'nhi', 'nhi_er', 'nh2', 'nh2_er', 'nh_co', 'nhco_er', 'nh_d', 'nhd_er']
...
def nh_factor_vs_nhi():
	data    = read_dust_nh_nhi(fname = 'result/nh_16src_from_dust.txt')
	src1    = data['src']
	idx1    = np.asarray(data['indx'])
	nhi1    = np.asarray(data['nhi'])
	nhi_er1 = np.asarray(data['nhi_er'])
	nh1     = np.asarray(data['nh_co'])
	nh_er1  = np.asarray(data['nhco_er'])
	...
	dat     = read_planck_nh_nhi(fname = 'sub_data/26src_no_co_nhi_and_uncertainties_full.txt')
	src2    = dat['src']
	...
	trace2 = fig.error_bar(x2,y2,label='Factor $f = N_{H}$/$N_{HI}$, $N_{H}$ from thermal dust, no CO',
	...
	trace3 = fig.error_bar(x1,y1,label='Factor $f = N_{H}$/$N_{HI}$, $N_{H}$ from thermal dust, CO present',
	...
	layout = dict(title  = 'Correlation between Factor $f = N_{H}$/$N_{HI}$ and HI column density $N_{HI}$ \nalong 42 lines-of-sight',
	              ...
	              text   = [dict(loc=[0.2,0.4],text='a = '+str(m)+'$\pm$'+str(ea) +',  b = '+str(b)+'$\pm$'+str(eb),color='blue',fontsize=17),
	              			...
	              			dict(loc=[0.01,8.0],text='CO present where N(HI) is high',color='blue',fontsize=17),
