...
## Read info of 78 sources #
 ...
 # version 04/10/2016
 # Author Van Hiep ##
def read_lb_nhi_30src_claire(fname = '../oh/sponge/sub_data/30src_claire.txt'):
...
def plot_patches(map_file, info):
	...
	# offset = 0.1 # Check structure in different scale
	# hp.cartview(tau_map, title=map_file, coord='G', unit='',
	# 			norm='hist', xsize=800, lonra=[66.-offset-0.1*offset,66.+offset+0.1*offset], latra=[8.-offset-0.1*offset,8.+offset+0.1*offset],
	...
	for i in range(0,len(src)):
		...
		if (err_map[pix] >= 6.9e-11 and err_map[pix] <= 0.00081): # Checked Invalid error, range = [4.82551e-10, 0.000144424]
		...
		for x in pl.frange(l-offset, l+offset, dd):
			for y in pl.frange(b-offset, b+offset, dd):
				...
				if ( (((x-l)**2 + (y-b)**2) <= offset**2) ):
					# hp.projtext(x, y, 'bo', lonlat=True, coord='G')
...
def cal_nh_from_dust(map_file, info):
	...
	for i in range(0, len(src)):
		...
		print("{}  {}\t{:08.4f}  {:08.4f}   {}   {}   {}   {}"
			.format(i, info['src'][i],l,b, info['nhi'][i], info['nhi_er'][i], round(nh_i*1e6,4), round(planck_sd,4)    ))
...
## Infor of 30 src from Claire paper ##
info = read_lb_nhi_30src_claire(fname = '../oh/sponge/sub_data/30src_claire.txt')
