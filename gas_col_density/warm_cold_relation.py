...
def get_gas_column_density(map_file, src_num, info):
	...
	for i in range(0,src_num):
		...
		# hp.cartview(tau_map, title=info['src'][i]+'('+str(info['l'][i])+','+str(info['b'][i])+') - '+map_file, coord='G', unit='', rot=[l,b],
		...
		l_offset = offset/np.cos(b*deg2rad)
		for x in pl.frange(l-l_offset, l+l_offset, dd):
			for y in pl.frange(b-offset, b+offset, dd):
				# hp.projtext(x, y, str(1), lonlat=True, coord='G') 
		...
		fukui_sd  = (sd2_tau353*fukui_cf)/nhi_i

		planck_sd = (sd2_tau353/tau353[i])**2 + (pl_fact_err/planck_cf)**2
		planck_sd = (nh_i*planck_sd**0.5)/nh_i
		...
		print("{0}\t{1}\t\t{2}\t\t{3}\t\t{4}\t\t{5}\t\t{6}\t\t{7}\t\t{8}\t\t{9}\t\t{10}"
			.format(i, info['src'][i], round((nhi[i])*1e6, 2), round((fukui_sd), 2), round((nh[i])*1e6, 2), round((planck_sd), 2), nhi_hi, wnm, cnm, rat1, rat2))
...
# Plot the relation between Warm and Cold Neutral Medium #
warm_cold_relation(info)