...
## Class - linear fit using MCMC with uniform Distribution of priors (alpha and beta) ##
 ...
 # Using, eg:
 # 		lfit        = mclfit()
 # 		pymc_trace  = lfit.fit(xdata,ydata,yerr,arange=[-100.,100.],brange=[-100.,100],print_stats = False )
...
class mclfit:
	...
	## MCMC linear fit ##
	 ...
	 # author Nguyen Van Hiep
	 ##
	def fit(self,x,y,yerr,arange=[-100.,100],brange=[-100.,100],print_stats = False):
		S = pymc.MCMC(self.model(x,y,yerr,arange,brange))
	...
	## Define the model/function to be fitted. ##
	 ...
	 # author Nguyen Van Hiep
	 ##
	def model(self,x,y,yerr,arange,brange):
		##
		#  func = alpha*x + beta
		##

		## alpha, beta are uniformly distributed ##
		alpha = pymc.Uniform('alpha', arange[0], arange[1] )
