...
class gfit:
	...
	def get_tb(self,x,bg,tau,v0,wid,tex,cont):
		...
		for i in range(ng):
			arrtau[:, i] = self.gfuncs(x,t_bg,amp[i],cntr[i],width[i])

		sumtau = arrtau.sum(1) #2048

		exp_sumtau = np.exp(-sumtau)
		tb_cont    = cont*exp_sumtau
		# TO HERE: 1st fitting, opacity spectrum e(-tau) where, tau = sum of Gaussians
	...
	## Get expeted Tb ##
	 ...
	 # version 08/2016 
	 # author Nguyen Van Hiep ##
	def get_abs_gfuncs(self,x,bg,tau,v0,wid,cont):
		# REARRANGE CLOUDS IN ORDER OF 'ORDER'.
		t_bg    = bg
		amp     = list(tau) # copy
		cntr    = list(v0)  # copy
		...
		sumtau = arrtau.sum(1) #2048 sum of N gaussians

		exp_sumtau = np.exp(-sumtau) # exp(-tau) from the [sum of N gaussians]
		tb_cont    = cont*exp_sumtau
		# TO HERE: 1st fitting, opacity spectrum e(-tau) where, tau = sum of Gaussians
	...
	##  ##
	 ...
	 # version 08/2016 
	 # author Nguyen Van Hiep ##
	def fit(self,vdata, tdata, vrangeid, cont, tbg, tau, v0, wid, ts,
		...
		while (repeat == 1):
			...
			# EVALUATE tbg DERIVATIVE: Neu tbg1 thay doi 1 luong nao do, xem thu Tc thay doi the nao
			xdel = np.float64(0.0000025)
			tbg1 = tbg1 + xdel
			tb_totplus = self.get_abs_gfuncs(xdat, \
				tbg1, tau1, v01, self.listxnum(wid1,1./dp600), ts1, cont1)
			...
			for ng in range(ngfit):
				...
				tau1[ng] = tau1[ng] + xdel  # back to tau1[ng]
			...
			t_predicted = self.get_abs_gfuncs(xdat, \
					tbg1, tau1, v01, self.listxnum(wid1,1./dp600), ts1, cont1)
			...
			## afull is array of delta of each params
